 ...
 */
function ppc_get_primary_term_posts( $term, $taxonomy = '', $args = array() ) {
	return PPC_Primary_Term_Query::get_posts_by_primary_term( $term, $taxonomy, $args );
}
 ...
 */
function ppc_primary_term_shortcode( $atts ) {
	// Get shortcode args.
	$args = shortcode_atts(
		array(
			'term'           => 0,
	...
	$primary_term_posts = ppc_get_primary_term_posts( $term, $args['taxonomy'], $post_query_args );

	// Action hook to display queries posts in themes.
	do_action( 'ppc_display_primary_term_posts', $primary_term_posts );
}
add_shortcode( 'ppc_primary_term_posts', 'ppc_primary_term_shortcode' );

/**
 * Register PPC meta for REST API.
 *
 * @return void
 */
function ppc_register_meta_for_rest() {
...
add_action( 'init', 'ppc_register_meta_for_rest', 9999 );
